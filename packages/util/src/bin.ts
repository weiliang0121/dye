import {ascending, extent, nice, tickIncrement, ticks} from './array';
import {isArr, isNum, isUndef} from './normal';
import {Bisector} from './bisector';
import {identity} from './function';

import type {GF} from '@dye/types';

const sturgesRule = (V: any[]) => Math.max(1, Math.ceil(Math.log2(V.length) + 1));
...
export class Bin {
  ...
  bin(data: any[]) {
    ...
    const bins = Array.from({length: T.length + 1}, (_, i) => {
      const bin = [] as unknown as any[] & {x0: number; x1: number; total: number};
      bin.x0 = i > 0 ? T[i - 1] : x0;
