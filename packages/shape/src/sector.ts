...
import {isNum, isUndef} from '@dye/util';
...
const tl = (t: number, t0: number, r: number, rc: number): Tangent => {
  const rc0 = (r * Math.sin(t / 2)) / (1 + Math.sin(t / 2));
  if (rc > rc0) rc = rc0;
  const ot = Math.asin(rc / (r - rc));
  const t1 = (t0 + ot) % (2 * Math.PI);
  const r0 = (r - rc) * Math.cos(ot);
  const x0 = r0 * Math.cos(t0);
  const y0 = r0 * Math.sin(t0);
  const x1 = r * Math.cos(t1);
  const y1 = r * Math.sin(t1);
  const cx = (r - rc) * Math.cos(t1);
  const cy = (r - rc) * Math.sin(t1);
  return {ot, cx, cy, x0, y0, x1, y1, rc};
};
const tr = (t: number, t0: number, r: number, rc: number): Tangent => {
  const rc0 = (r * Math.sin(t / 2)) / (1 + Math.sin(t / 2));
  if (rc > rc0) rc = rc0;
  const ot = Math.asin(rc / (r - rc));
  const t1 = (t0 - ot) % (2 * Math.PI);
...
const bl = (t: number, t0: number, r: number, rc: number): Tangent => {
  const rc0 = (r * Math.sin(t / 2)) / (1 - Math.sin(t / 2));
  if (rc > rc0) rc = rc0;
  const ot = Math.asin(rc / (r + rc));
  const t1 = (t0 + ot) % (2 * Math.PI);
...
const br = (t: number, t0: number, r: number, rc: number): Tangent => {
  const rc0 = (r * Math.sin(t / 2)) / (1 - Math.sin(t / 2));
  if (rc > rc0) rc = rc0;
  const ot = Math.asin(rc / (r + rc));
  const t1 = (t0 - ot) % (2 * Math.PI);
...
export const createSectorRounded = (path: Path, options: SectorOptions) => {
  ...
  if (dt < 1e-6) return;
  const c01 = tl(dt, t0, r1, rc01);
  const c11 = tr(dt, t1, r1, rc11);
  ...
  if (r0 === 0) {
    ...
    const c00 = bl(dt, t0, r0, rc00);
    const c10 = br(dt, t1, r0, rc10);
