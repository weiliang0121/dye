...
export const createArea = (path: Path, options: AreaOptions) => {
  ...
  path.L(x, y);
  if (Reflect.has(curveMap, lowerCurve)) curveMap[upperCurve](path, lowerPoints.reverse(), false);
  path.Z();
};
...
export const createSegmentArea = (path: Path, options: SegmentAreaOptions) => {
  ...
  for (let i = 0; i < length; i++) {
    const upperPoints = upperSegments[i];
    const lowerPoints = lowerSegments[i];
    createArea(path, {upperPoints, lowerPoints, curve, upperCurve, lowerCurve});
  }
};
